"""
core/metrics.py — KPI calculation layer  v2
All numbers produced here. Zero UI imports.
"""
...
def pipeline_summary(df: pd.DataFrame) -> dict[str, Any]:
    ...
    delayed    = int(df["sla_breach"].sum())
    total_cost = round(float(df["cost"].sum(skipna=True)), 2)
    avg_age    = round(float(df["days_in_stage"].dropna().mean()), 1) if df["days_in_stage"].notna().any() else 0.0
    return dict(total=total, received=received, active=active,
...
def status_distribution(df: pd.DataFrame) -> pd.DataFrame:
    return (df.groupby("status_label").size()
...
def category_breakdown(df: pd.DataFrame) -> pd.DataFrame:
    grp = (
        ...
        .agg(count=("ta_ref","count"), total_cost=("cost","sum"), delayed=("sla_breach","sum"))
        .reset_index().rename(columns={"category_name":"category"})
...
def supplier_performance(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    primary = df[df["supplier"].str.strip().ne("") & df["supplier"].notna()]
    for _, row in primary.iterrows():
        rows.append(dict(supplier=row["supplier"], ta_ref=row["ta_ref"],
                         cost=row["cost"] if pd.notna(row["cost"]) else 0,
                         on_time=not row["sla_breach"]))
        for sub in (row.get("sub_orders") or []):
            if sub.get("supplier"):
                rows.append(dict(supplier=sub["supplier"], ta_ref=row["ta_ref"],
...
def timeline_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    monthly = (ts.groupby("month").size().reset_index(name="requisitions")
                 .sort_values("month"))
...
def age_distribution(df: pd.DataFrame) -> pd.DataFrame:
    """Days-in-stage distribution across active (non-received, non-cancelled) items."""
    active = df[~df["status"].isin(["RECEIVED","CANCELLED"]) & df["days_in_stage"].notna()].copy()
    if active.empty:
        return pd.DataFrame(columns=["ta_ref","description","days_in_stage","status_label"])
    return active[["ta_ref","description","days_in_stage","status_label"]].sort_values("days_in_stage", ascending=False)


def cost_by_month(df: pd.DataFrame) -> pd.DataFrame:
    """Monthly spend from order_date for items that have a cost."""
    ts = df[df["order_date"].notna() & df["cost"].notna() & (df["cost"] > 0)].copy()
    if ts.empty:
        return pd.DataFrame(columns=["month_label","cost"])
    ts["month"] = ts["order_date"].dt.to_period("M").dt.to_timestamp()
    monthly = (ts.groupby("month")["cost"].sum().reset_index()
                 .sort_values("month"))
    monthly["month_label"] = monthly["month"].dt.strftime("%b %Y")
    return monthly
